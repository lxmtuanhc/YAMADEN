...
/* ========== MAIN APP SPA (after login only) ========== */
(function(){
  const MAIN_TABS = [
    { id: "home", label: "Trang chủ", icon: "home" },
    { id: "requests", label: "Yêu cầu", icon: "file" },
    { id: "quotes", label: "Báo giá", icon: "chart" },
    { id: "schedules", label: "Lịch hẹn", icon: "clock" },
  ...
  const requestSeed = [
    {
      ...
      project: "Tòa nhà văn phòng YAMADA",
      date: "16/05/2024 10:30",
      status: "processing",
      creator: "Nguyễn Văn A",
      category: "Sự cố điện",
      location: "Tòa nhà văn phòng YAMADA, 1-2-3 Yamada, Tokyo",
      description: "Hệ thống điện khu vực phòng họp thường bị ngắt điện đột ngột. Đề nghị kiểm tra và khắc phục.",
      preferredTime: "20/05/2024 (Thứ 2) 09:00 - 11:00",
      images: ["Ảnh 1", "Ảnh 2"]
    },
    {
      id: "RQ-2024-0415",
      ...
      description: "Cần bổ sung 6 ổ cắm tại khu vực làm việc mới.",
      preferredTime: "18/05/2024 13:00 - 15:00",
      images: ["Ảnh 1"]
    },
    {
      id: "RQ-2024-0380",
      ...
      project: "Tòa nhà văn phòng YAMADA",
      date: "02/05/2024 09:15",
      status: "completed",
      creator: "Nguyễn Văn A",
      category: "Bảo trì",
      location: "Tầng 2",
      description: "Kiểm tra định kỳ hệ thống điều hòa trung tâm.",
      preferredTime: "05/05/2024 09:00",
      images: []
    },
    {
      id: "RQ-2024-0325",
      title: "Thay thế bóng đèn chiếu sáng",
      project: "Kho vật tư YAMADA",
      date: "25/04/2024 16:45",
      status: "cancelled",
      creator: "Nguyễn Văn A",
      category: "Chiếu sáng",
      location: "Kho vật tư",
      description: "Thay các bóng đèn khu vực lối đi.",
      preferredTime: "30/04/2024",
      images: []
    }
  ];
  ...
  const quoteSeed = [
    {
      id: "Q-2024-0385",
      project: "Lắp thêm 6 ổ cắm và 6 dây điện",
      total: 29370,
      validUntil: "25/05/2024",
      status: "waiting",
      items: [
        { name: "Ổ cắm Panasonic WN1001SW", qty: 6, price: 1200 },
        { name: "Dây điện 2.0mm", qty: 50, price: 150 },
        { name: "Nhân công lắp đặt", qty: 1, price: 12000 },
        { name: "Kiểm tra và hoàn thiện", qty: 1, price: 0 }
      ]
    },
    {
      id: "Q-2024-0312",
      project: "Sửa chập phòng bếp",
      total: 18700,
      validUntil: "20/05/2024",
      status: "approved",
      items: [
        { name: "Kiểm tra đường dây", qty: 1, price: 6500 },
        { name: "Thay breaker", qty: 1, price: 8200 },
        { name: "Vệ sinh tủ điện", qty: 1, price: 2300 }
      ]
    },
    {
      id: "Q-2024-0288",
      project: "Thay automat tổng 40A",
      total: 24200,
      validUntil: "15/05/2024",
      status: "rejected",
      items: [
        { name: "Automat 40A", qty: 1, price: 13000 },
        { name: "Nhân công", qty: 1, price: 9000 }
      ]
    }
  ];
  ...
  const schedules = [
    {
      id: "SC-001",
      date: "20/05/2024 (Thứ 2)",
      time: "09:00 - 11:00",
      technician: "Kobayashi Taro",
      project: "Tòa nhà văn phòng YAMADA",
      status: "upcoming",
      steps: ["Tiếp nhận yêu cầu", "Xác nhận lịch hẹn", "Kỹ thuật viên đang trên đường", "Hoàn thành"]
    },
    {
      id: "SC-002",
      date: "24/05/2024 (Thứ 6)",
      time: "14:00 - 16:00",
      technician: "Tanaka Satoshi",
      project: "Kho vật tư YAMADA",
      status: "waiting",
      steps: ["Tiếp nhận yêu cầu", "Chờ xác nhận lịch hẹn", "Kỹ thuật viên đang trên đường", "Hoàn thành"]
    }
  ];
  ...
  const state = {
    ...
    requestSearch: "",
    quoteSearch: "",
    requests: loadLocal("yamaden_spa_requests", requestSeed),
    quotes: loadLocal("yamaden_spa_quotes", quoteSeed)
  };
  ...
  function AppHeader(title, subtitle, backTo) {
    return `
      <div class="app-main-header">
        <div>
          <div class="app-main-title">${esc(title)}</div>
          ${subtitle ? `<div class="app-main-subtitle">${esc(subtitle)}</div>` : ""}
        </div>
        ${backTo ? `<button class="app-icon-button" type="button" onclick="mainNavigate('${esc(backTo)}')" aria-label="Back"><span class="icon">${icon("logout")}</span></button>` : `<button class="app-icon-button" type="button" aria-label="Notifications"><span class="icon">${icon("bell")}</span></button>`}
      </div>
    `;
  }

  function SearchBar(value, placeholder, onInputName) {
    return `
      <div class="app-search-row">
        <div class="app-search"><span class="icon">${icon("search")}</span><input value="${esc(value)}" placeholder="${esc(placeholder)}" oninput="${onInputName}(this.value)"></div>
        <button class="app-icon-button" type="button" aria-label="Filter"><span class="icon">${icon("tool")}</span></button>
      </div>
    `;
  }

  function FilterTabs(items, active, handler) {
    return `<div class="app-filter-tabs">${items.map(item => `<button type="button" class="app-filter-tab ${active === item.id ? "active" : ""}" onclick="${handler}('${esc(item.id)}')">${esc(item.label)}</button>`).join("")}</div>`;
  }
  ...
  function RequestCard(item) {
    return `
      <div class="app-list-card" onclick="mainNavigate('request-detail','${esc(item.id)}')">
        <div class="app-list-icon">${icon("file")}</div>
        <div>
          <div class="app-card-top"><div class="app-list-meta">${esc(item.id)}</div>${statusBadge(item.status)}</div>
          <div class="app-list-title">${esc(item.title)}</div>
          <div class="app-list-sub">${esc(item.project)}<br>${esc(item.date)}</div>
        </div>
        <div class="app-list-meta">&gt;</div>
      </div>
    `;
  }
  ...
  function QuoteCard(item) {
    return `
      <div class="app-list-card" onclick="mainNavigate('quote-detail','${esc(item.id)}')">
        <div class="app-list-icon">${icon("chart")}</div>
        <div>
          <div class="app-card-top"><div class="app-list-meta">${esc(item.id)}</div>${statusBadge(item.status)}</div>
          <div class="app-list-title">${esc(item.project)}</div>
          <div class="app-list-sub">${money(item.total)} · Hạn hiệu lực: ${esc(item.validUntil)}</div>
        </div>
        <div class="app-list-meta">&gt;</div>
      </div>
    `;
    ...
    return `
      <div class="app-card">
        <div class="app-card-top">
          <div>
            <div class="app-list-title">${esc(item.date)}</div>
            <div class="app-list-meta">${esc(item.time)}</div>
          </div>
          ${statusBadge(item.status)}
        </div>
        <div class="app-info-row"><div class="app-info-label">Kỹ thuật viên</div><div class="app-info-value">${esc(item.technician)}</div></div>
        <div class="app-info-row"><div class="app-info-label">Công trình</div><div class="app-info-value">${esc(item.project)}</div></div>
        ${Timeline(item.steps, item.status === "waiting" ? 1 : 2)}
        <div class="app-button-row">
          <button class="app-outline-button" type="button">Gọi kỹ thuật viên</button>
          <button class="app-primary-button" type="button">Chat hỗ trợ</button>
        </div>
      </div>
    `;
  }

  function Timeline(items, activeIndex) {
    return `
      <div class="app-section-label">Tiến độ xử lý</div>
      <div class="app-timeline">
        ${items.map((item, index) => `
          <div class="app-timeline-item ${index <= activeIndex ? "done" : ""}">
            <div class="app-timeline-dot"></div>
            <div>
              <div class="app-timeline-title">${esc(item)}</div>
              <div class="app-timeline-time">${index <= activeIndex ? "Đã cập nhật" : "Chưa bắt đầu"}</div>
            </div>
          </div>
        `).join("")}
      </div>
  ...
  function renderHome() {
    ...
    return `
      ${AppHeader("Trang chủ", "YAMADEN Support")}
      <div class="app-home-hero">
        <div class="app-home-user">
          <div class="app-avatar">${icon("user")}</div>
          <div>
            <div class="app-list-title">Xin chào,<br>${esc(name)}</div>
            <div class="app-list-meta">Cảm ơn bạn đã tin tưởng YAMADEN.</div>
          </div>
        </div>
        <button class="app-primary-button" type="button" onclick="mainNavigate('request-new')">+ Tạo yêu cầu mới</button>
        <div class="app-home-grid">
          <div class="app-stat"><b>${state.requests.filter(r => r.status === "processing").length}</b><span>Đang xử lý</span></div>
          <div class="app-stat"><b>${state.quotes.length}</b><span>Báo giá</span></div>
          <div class="app-stat"><b>${schedules.length}</b><span>Lịch hẹn</span></div>
        </div>
      </div>
      <div class="app-card">
        <div class="app-section-label" style="margin-top:0">Yêu cầu hiện tại</div>
        ${RequestCard(state.requests[0])}
      </div>
      <div class="app-card">
        <div class="app-section-label" style="margin-top:0">Thông báo mới</div>
        <div class="app-list-meta">Báo giá ${esc(state.quotes[0].id)} đang chờ xác nhận.</div>
      </div>
    `;
  }

  function renderRequests() {
    const list = state.requests.filter(item => {
      const matchesFilter = state.requestFilter === "all" || item.status === state.requestFilter;
      const keyword = state.requestSearch.trim().toLowerCase();
      const matchesSearch = !keyword || [item.id, item.title, item.project].some(v => String(v).toLowerCase().includes(keyword));
      return matchesFilter && matchesSearch;
    });
    return `
      ${AppHeader("Yêu cầu")}
      <button class="app-primary-button" type="button" onclick="mainNavigate('request-new')">+ Tạo yêu cầu mới</button>
      ${SearchBar(state.requestSearch, "Tìm kiếm theo mã yêu cầu...", "setRequestSearch")}
      ${FilterTabs(REQUEST_FILTERS, state.requestFilter, "setRequestFilter")}
      <div class="app-section-label">Lịch sử yêu cầu</div>
      <div class="app-list">${list.length ? list.map(RequestCard).join("") : EmptyState("Chưa có yêu cầu phù hợp.")}</div>
    `;
  }

  function renderRequestNew() {
    return `
      ${AppHeader("Tạo yêu cầu mới", "", "requests")}
      <div class="app-form-card">
        <div class="app-field"><label>Danh mục yêu cầu *</label><select id="newRequestCategory"><option>Sự cố điện</option><option>Thi công</option><option>Bảo trì</option><option>Báo giá</option></select></div>
        <div class="app-field"><label>Tiêu đề yêu cầu *</label><input id="newRequestTitle" placeholder="Nhập tiêu đề ngắn gọn"></div>
        <div class="app-field"><label>Mô tả chi tiết *</label><textarea id="newRequestDesc" placeholder="Nhập nội dung chi tiết về vấn đề, hiện trạng..."></textarea></div>
        <div class="app-field"><label>Địa điểm *</label><input id="newRequestLocation" placeholder="Nhập địa chỉ công trình / địa điểm"></div>
        <div class="app-field"><label>Thời gian mong muốn</label><input id="newRequestTime" placeholder="Chọn thời gian"></div>
        <div class="app-field"><label>Hình ảnh đính kèm</label><div class="app-upload-box">Chọn ảnh hoặc kéo thả vào đây<br>Tối đa 5 ảnh</div></div>
        <button class="app-primary-button" type="button" onclick="submitMockRequest()">Gửi yêu cầu</button>
      </div>
    `;
  }

  function renderRequestDetail(id) {
    const item = state.requests.find(r => r.id === id) || state.requests[0];
    return `
      ${AppHeader("Chi tiết yêu cầu", "", "requests")}
      <div class="app-card">
        <div class="app-card-top"><div class="app-list-title">${esc(item.id)}</div>${statusBadge(item.status)}</div>
        <div class="app-detail-grid">
          <div class="app-info-row"><div class="app-info-label">Tiêu đề</div><div class="app-info-value">${esc(item.title)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Công trình</div><div class="app-info-value">${esc(item.project)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Ngày gửi</div><div class="app-info-value">${esc(item.date)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Người tạo</div><div class="app-info-value">${esc(item.creator)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Danh mục</div><div class="app-info-value">${esc(item.category)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Địa điểm</div><div class="app-info-value">${esc(item.location)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Mô tả</div><div class="app-info-value">${esc(item.description)}</div></div>
          <div class="app-info-row"><div class="app-info-label">Thời gian</div><div class="app-info-value">${esc(item.preferredTime)}</div></div>
        </div>
      </div>
      <div class="app-card">
        <div class="app-section-label" style="margin-top:0">Hình ảnh đính kèm</div>
        <div class="app-photo-row">${item.images.length ? item.images.map(img => `<div class="app-photo">${esc(img)}</div>`).join("") : `<div class="app-list-meta">Không có hình ảnh</div>`}</div>
      </div>
      <div class="app-card">${Timeline(["Đã tiếp nhận", "Đang xử lý", "Chờ phản hồi", "Hoàn thành"], item.status === "completed" ? 3 : item.status === "waiting" ? 2 : 1)}</div>
      <button class="app-primary-button" type="button">Liên hệ hỗ trợ</button>
    `;
  }

  function renderQuotes() {
    const list = state.quotes.filter(item => {
      const matchesFilter = state.quoteFilter === "all" || item.status === state.quoteFilter;
      const keyword = state.quoteSearch.trim().toLowerCase();
      const matchesSearch = !keyword || [item.id, item.project].some(v => String(v).toLowerCase().includes(keyword));
      return matchesFilter && matchesSearch;
    });
    return `
      ${AppHeader("Báo giá")}
      ${SearchBar(state.quoteSearch, "Tìm kiếm theo mã báo giá...", "setQuoteSearch")}
      ${FilterTabs(QUOTE_FILTERS, state.quoteFilter, "setQuoteFilter")}
      <div class="app-section-label">Danh sách báo giá</div>
      <div class="app-list">${list.length ? list.map(QuoteCard).join("") : EmptyState("Chưa có báo giá phù hợp.")}</div>
    `;
  }
    ...
    return `
      ...
      <div class="app-card">
        <table class="quote-table">
          <thead><tr><th>Hạng mục</th><th>SL</th><th>Đơn giá</th><th>Thành tiền</th></tr></thead>
          <tbody>${item.items.map(row => `<tr><td>${esc(row.name)}</td><td>${row.qty}</td><td>${money(row.price)}</td><td>${money(row.qty * row.price)}</td></tr>`).join("")}</tbody>
        </table>
        <div class="quote-summary">
          <div class="quote-summary-row"><span>Tạm tính</span><b>${money(subTotal)}</b></div>
          <div class="quote-summary-row"><span>Thuế VAT (10%)</span><b>${money(vat)}</b></div>
          <div class="quote-summary-row total"><span>Tổng cộng</span><b>${money(item.total)}</b></div>
        </div>
      </div>
      <div class="app-button-row">
        <button class="app-outline-button" type="button">Yêu cầu chỉnh sửa</button>
        <button class="app-primary-button" type="button">Đồng ý báo giá</button>
      </div>
    `;
  }
  ...
  function renderSchedules() {
    return `
      ${AppHeader("Lịch hẹn")}
      <div class="app-section-label">Danh sách lịch hẹn sắp tới</div>
      <div class="app-list">${schedules.map(ScheduleCard).join("")}</div>
    `;
  }
    ...
    return `
      ...
      <div class="app-card">
        <div class="app-home-user">
          <div class="app-avatar">${icon("user")}</div>
          <div>
            <div class="app-list-title">${esc(user.name || "Nguyễn Văn A")}</div>
            <div class="app-list-meta">${esc(user.phone || "090 1234 567")}<br>${esc(user.email || "nguyenvana@example.com")}</div>
          </div>
        </div>
      </div>
      <div class="account-menu">
        ${menu.map(row => `<div class="account-menu-row"><span class="account-menu-icon">${icon(row[0])}</span><span>${esc(row[1])}</span><span>&gt;</span></div>`).join("")}
        <button class="app-danger-button" type="button" onclick="userLogout()">Đăng xuất</button>
      </div>
    `;
  }

  function ensureMainScreen() {
    let screen = $("mainAppSection");
    if (!screen) {
      screen = document.createElement("section");
      screen.id = "mainAppSection";
      screen.className = "main-app-screen";
      const main = document.querySelector(".main");
      const notice = $("notice");
      if (notice && notice.parentNode) notice.insertAdjacentElement("afterend", screen);
      else main?.prepend(screen);
    }
    return screen;
  }

  function hideLegacySections() {
    ["accountSection", "homeSection", "servicesSection", "sendSection", "checkSection", "settingsSection"].forEach(id => {
      const el = $(id);
  ...
  function renderBottomNav(activeTab) {
    ...
    if (!nav) return;
    nav.innerHTML = MAIN_TABS.map(tab => `
      <button id="mainTab-${tab.id}" class="${activeTab === tab.id ? "active" : ""}" type="button" onclick="mainNavigate('${tab.id}')">
        <span class="icon">${icon(tab.icon)}</span><span>${esc(tab.label)}</span>
      </button>
    `).join("");
  }

  function tabForRoute(route) {
    if (route.startsWith("request")) return "requests";
  ...
  function renderRoute() {
    ...
    const app = document.querySelector(".app");
    app?.classList.remove("auth-login-mode");
    app?.classList.add("main-spa-mode");
    hideLegacySections();
    const screen = ensureMainScreen();
    ...
    screen.innerHTML = (views[state.route] || renderHome)();
    renderBottomNav(tabForRoute(state.route));
  ...
  function parseHash() {
    ...
    if (!raw) return { route: "home", id: "" };
    const parts = raw.split("/").filter(Boolean);
    return { route: parts[0] || "home", id: parts[1] || "" };
  }

  window.mainNavigate = function(route, id) {
  ...
  window.setRequestFilter = function(filter) {
    state.requestFilter = filter;
    renderRoute();
  };

  window.setRequestSearch = function(value) {
    state.requestSearch = value;
    renderRoute();
  };

  window.setQuoteFilter = function(filter) {
    state.quoteFilter = filter;
    renderRoute();
  };

  window.setQuoteSearch = function(value) {
    state.quoteSearch = value;
    renderRoute();
  };

  window.submitMockRequest = function() {
    const title = $("newRequestTitle")?.value.trim();
    const description = $("newRequestDesc")?.value.trim();
    const location = $("newRequestLocation")?.value.trim();
    if (!title || !description || !location) {
      showToast ? showToast("Vui lòng nhập đầy đủ thông tin.") : alert("Vui lòng nhập đầy đủ thông tin.");
      return;
    }
    const user = getUser();
    const id = "RQ-2024-" + String(Math.floor(1000 + Math.random() * 8999));
    const request = {
      id,
      title,
      project: user.projectName || "Công trình YAMADEN",
      date: new Date().toLocaleString("vi-VN", { hour12: false }),
      status: "processing",
      creator: user.name || "Khách hàng",
      category: $("newRequestCategory")?.value || "Yêu cầu",
      location,
      description,
      preferredTime: $("newRequestTime")?.value || "Chưa chọn",
      images: ["Ảnh mới"]
    };
    state.requests.unshift(request);
    saveLocal("yamaden_spa_requests", state.requests);
    showToast && showToast("Đã tạo yêu cầu.");
    window.mainNavigate("request-detail", id);
  };
  ...
  window.showTab = function(tab) {
    ...
    if (tab === "send" && isLoggedIn()) {
      window.mainNavigate("request-new");
      return;
    }
    if (tab === "check" && isLoggedIn()) {
      window.mainNavigate("requests");
      return;
    }
    if (tab === "settings" && isLoggedIn()) {
      window.mainNavigate("account");
      return;
    }
    ...
    document.querySelector(".app")?.classList.remove("main-spa-mode");
    $("mainAppSection")?.classList.remove("active");
    if (oldShowTab) oldShowTab(tab);
  };
