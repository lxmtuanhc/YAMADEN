...
(function(){
  ...
  const txt={ja:{title:"\u30b9\u30bf\u30c3\u30d5\u30d7\u30ed\u30d5\u30a3\u30fc\u30eb",sub:"\u90e8\u9580\u30fb\u62c5\u5f53\u4f5c\u696d\u30fb\u30a2\u30d0\u30bf\u30fc\u3092\u7ba1\u7406\u3057\u307e\u3059\u3002",add:"\u65b0\u898f\u30d7\u30ed\u30d5\u30a3\u30fc\u30eb\u8ffd\u52a0",avt:"\u30a2\u30d0\u30bf\u30fc",pickAvatar:"\u5199\u771f\u3092\u9078\u629e",name:"\u540d\u524d",email:"Email",dept:"\u90e8\u7f72",deptVi:"\u30d9\u30c8\u30ca\u30e0\u8a9e",work:"\u62c5\u5f53\u4f5c\u696d\u5185\u5bb9",workPlaceholder:"\u696d\u52d9\u5185\u5bb9\u3092\u5165\u529b",addWork:"\u8ffd\u52a0",workEmpty:"\u696d\u52d9\u5185\u5bb9\u306f\u3042\u308a\u307e\u305b\u3093\u3002",hint:"\u5165\u529b\u3057\u3066\u8ffd\u52a0\u3001\u307e\u305f\u306f\u30ab\u30f3\u30de\u30fb\u6539\u884c\u3067\u8907\u6570\u8cbc\u308a\u4ed8\u3051\u3067\u304d\u307e\u3059\u3002",edit:"\u7de8\u96c6",delete:"\u524a\u9664",cancel:"\u30ad\u30e3\u30f3\u30bb\u30eb",save:"\u4fdd\u5b58",noData:"\u30c7\u30fc\u30bf\u304c\u3042\u308a\u307e\u305b\u3093",basic:"\u57fa\u672c\u60c5\u5831",history:"\u4f9d\u983c\u5c65\u6b74",requestId:"ID",content:"\u5185\u5bb9",status:"\u30b9\u30c6\u30fc\u30bf\u30b9",wait:"\u5f85\u6a5f\u6642\u9593",registered:"\u767b\u9332\u65e5"},vi:{title:"H\u1ed3 s\u01a1 nh\u00e2n vi\u00ean",sub:"Qu\u1ea3n l\u00fd b\u1ed9 ph\u1eadn, n\u1ed9i dung c\u00f4ng vi\u1ec7c v\u00e0 avatar.",add:"Th\u00eam h\u1ed3 s\u01a1 m\u1edbi",avt:"Avatar",pickAvatar:"Ch\u1ecdn \u1ea3nh",name:"T\u00ean",email:"Email",dept:"B\u1ed9 ph\u1eadn l\u00e0m vi\u1ec7c",deptVi:"T\u00ean ti\u1ebfng Vi\u1ec7t",work:"N\u1ed9i dung c\u00f4ng vi\u1ec7c ph\u1ee5 tr\u00e1ch",workPlaceholder:"Nh\u1eadp n\u1ed9i dung c\u00f4ng vi\u1ec7c",addWork:"Th\u00eam",workEmpty:"Kh\u00f4ng c\u00f3 n\u1ed9i dung c\u00f4ng vi\u1ec7c.",hint:"Nh\u1eadp r\u1ed3i th\u00eam, ho\u1eb7c paste nhi\u1ec1u n\u1ed9i dung b\u1eb1ng d\u1ea5u ph\u1ea9y / xu\u1ed1ng d\u00f2ng.",edit:"S\u1eeda",delete:"X\u00f3a",cancel:"H\u1ee7y",save:"L\u01b0u",noData:"Kh\u00f4ng c\u00f3 d\u1eef li\u1ec7u",basic:"Th\u00f4ng tin",history:"L\u1ecbch s\u1eed y\u00eau c\u1ea7u",requestId:"ID",content:"N\u1ed9i dung",status:"Tr\u1ea1ng th\u00e1i",wait:"Th\u1eddi gian ch\u1edd",registered:"Ng\u00e0y \u0111\u0103ng k\u00fd"}};
  ...
  const normalizeWorkItems=value=>{const raw=Array.isArray(value)?value.join("\n"):String(value||"");const seen=new Set();const out=[];raw.split(/[,;、；\n\r]+/).map(x=>x.trim()).filter(Boolean).forEach(x=>{const k=normText(x);if(seen.has(k))return;seen.add(k);out.push(x)});return out};
  const parseTags=s=>normalizeWorkItems((Array.isArray(s?.workTags)&&s.workTags.length)?s.workTags:(Array.isArray(s?.tags)&&s.tags.length)?s.tags:(s?.workContent||s?.workContents||s?.jobContents||s?.responsibilities||s?.tasks||s?.work||s?.skills||""));
  ...
  const workChipsHtml=(list,limit=4)=>{const labels=(Array.isArray(list)?list:[]).map(workLabel).filter(Boolean);const visible=labels.slice(0,limit);const more=labels.length-visible.length;return labels.length?`<div class="staff-work-chip-list">${visible.map(x=>`<span class="staff-work-chip">${esc(x)}</span>`).join("")}${more?`<span class="staff-work-more">+${more}</span>`:""}</div>`:`<div class="staff-work-empty">${esc(T("workEmpty"))}</div>`};
  let staffWorkItemsFinal=[];
  function syncDeptNote(){}
  window.renderStaffWorkTagOptionsFinal=function(selected=staffWorkItemsFinal){const box=$("staffWorkTagBox");if(!box)return;staffWorkItemsFinal=normalizeWorkItems(selected);box.innerHTML=staffWorkItemsFinal.length?staffWorkItemsFinal.map((item,i)=>`<span class="staff-work-chip">${esc(workLabel(item))}<button type="button" class="staff-work-remove" data-remove-work="${i}" aria-label="remove">×</button></span>`).join(""):`<div class="staff-work-empty">${esc(T("workEmpty"))}</div>`;syncDeptNote()};
  window.addStaffWorkItemsFromInput=function(){const input=$("staffWorkInput");staffWorkItemsFinal=normalizeWorkItems(staffWorkItemsFinal.concat(normalizeWorkItems(input?.value||"")));if(input)input.value="";renderStaffWorkTagOptionsFinal(staffWorkItemsFinal)};
  window.removeStaffWorkItemFinal=function(index){staffWorkItemsFinal.splice(Number(index),1);renderStaffWorkTagOptionsFinal(staffWorkItemsFinal)};
  window.openStaffProfileForm=function(id=""){let old=$("staffFormModal");if(old)old.remove();const s=rows().find(x=>sid(x)===String(id));const d=deptOf(rawDept(s||{}));const modal=document.createElement("div");modal.id="staffFormModal";modal.className="staff-form-modal show";modal.onclick=e=>{if(e.target===modal)closeStaffProfileForm()};modal.innerHTML=`<div class="staff-form-box"><div class="staff-form-head"><div class="staff-form-title">${esc(T("title"))}</div><button class="staff-form-close" onclick="closeStaffProfileForm()"></button></div><div class="staff-form-body"><input type="hidden" id="staffProfileId" value="${esc(id)}"><input type="hidden" id="staffProfileAvatarUrl" value="${esc(avatarUrl(s))}"><div class="staff-avatar-field"><div class="staff-avatar-preview" id="staffAvatarPreview">${avatarUrl(s)?`<img src="${esc(avatarUrl(s))}" alt="">`:esc(initial(s))}</div><div class="staff-avatar-input-wrap"><label>${esc(T("avt"))}</label><input id="staffProfileAvatar" class="staff-avatar-file" type="file" accept="image/*"><button type="button" class="staff-avatar-pick" onclick="document.getElementById('staffProfileAvatar').click()">${esc(T("pickAvatar"))}</button></div></div><div class="staff-field"><label>${esc(T("name"))}</label><input id="staffProfileName" value="${esc(s?.name||"")}"></div><div class="staff-field"><label>${esc(T("email"))}</label><input id="staffProfileEmail" type="email" value="${esc(s?.email||"")}"></div><div class="staff-field"><label>${esc(T("dept"))}</label><select id="staffProfileDept">${deptOptions()}</select></div><div class="staff-field"><label>${esc(T("work"))}</label><div class="muted" style="font-size:12px;font-weight:800">${esc(T("hint"))}</div><div class="staff-work-input-row"><input id="staffWorkInput" placeholder="${esc(T("workPlaceholder"))}"><button type="button" class="staff-work-add" onclick="addStaffWorkItemsFromInput()">${esc(T("addWork"))}</button></div><div class="work-tag-box staff-work-chip-box" id="staffWorkTagBox"></div></div><div class="staff-form-actions"><button class="staff-cancel-profile" onclick="closeStaffProfileForm()">${esc(T("cancel"))}</button><button class="staff-save-profile" onclick="saveStaffProfileV8()">${esc(T("save"))}</button></div></div></div>`;document.body.appendChild(modal);$("staffProfileDept").value=d.ja;renderStaffWorkTagOptionsFinal(parseTags(s||{}));$("staffProfileAvatar").addEventListener("change",e=>{const f=e.target.files&&e.target.files[0];if(f)$("staffAvatarPreview").innerHTML=`<img src="${URL.createObjectURL(f)}" alt="">`});const input=$("staffWorkInput");if(input){input.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();addStaffWorkItemsFromInput()}});input.addEventListener("paste",()=>setTimeout(()=>addStaffWorkItemsFromInput(),0))}};
  window.closeStaffProfileForm=function(){$("staffFormModal")?.remove()};
  window.saveStaffProfileV8=async function(){const id=$("staffProfileId")?.value||"";const name=$("staffProfileName")?.value.trim()||"";if(!name)return;const dept=deptOf($("staffProfileDept")?.value||"").ja;const tags=normalizeWorkItems(staffWorkItemsFinal.concat(normalizeWorkItems($("staffWorkInput")?.value||"")));const fd=new FormData();fd.append("name",name);fd.append("email",$("staffProfileEmail")?.value.trim()||"");fd.append("phone","");fd.append("areas",dept);fd.append("department",dept);fd.append("skills",tags.join(", "));fd.append("workContent",tags.join(", "));fd.append("workTags",JSON.stringify(tags));fd.append("status","active");const keep=$("staffProfileAvatarUrl")?.value||"";if(keep)fd.append("avatar",keep);const file=$("staffProfileAvatar")?.files?.[0];if(file)fd.append("avatar",file);const res=await apiFetch(API+"/admin/staff"+(id?"/"+encodeURIComponent(id):""),{method:id?"PUT":"POST",body:fd});if(res.status===401)return authFailLogout();closeStaffProfileForm();loadStaff()};
  window.renderStaff=function(){const root=$("staffView");if(!root)return;root.innerHTML=`<div class="panel"><div class="staff-profile-shell"><div class="staff-profile-head"><div><div class="staff-profile-title">${esc(T("title"))}</div><div class="muted" style="font-size:12px;font-weight:750;margin-top:4px">${esc(T("sub"))}</div></div><button type="button" class="staff-add-btn">+ ${esc(T("add"))}</button></div><div class="staff-profile-list-head"><div>${esc(T("avt"))}</div><div>${esc(T("name"))}</div><div>${esc(T("dept"))}</div><div>${esc(T("email"))}</div><div>${esc(T("edit"))}</div><div>${esc(T("delete"))}</div></div><div class="staff-profile-list" id="staffProfileList"></div></div></div>`;const list=$("staffProfileList");list.innerHTML=rows().length?rows().map(s=>{const id=sid(s);return `<div class="staff-profile-row" data-staff-id="${esc(id)}"><div>${avatar(s)}</div><div><div class="staff-row-name">${esc(s.name||"-")}</div></div><div class="staff-dept-cell">${deptHtml(rawDept(s))}${workChipsHtml(parseTags(s),3)}</div><div class="staff-mail-cell staff-row-mail">${esc(s.email||"-")}</div><div class="staff-action-cell"><button type="button" class="staff-row-btn edit" data-edit-staff="${esc(id)}">${esc(T("edit"))}</button></div><div class="staff-action-cell"><button type="button" class="staff-row-btn delete" data-delete-staff="${esc(id)}">${esc(T("delete"))}</button></div></div>`}).join(""):`<div class="muted" style="padding:24px;text-align:center">${esc(T("noData"))}</div>`;const count=$("finalStaffCount");if(count)count.textContent=rows().length};
  window.openStaffDetail=function(id){const s=rows().find(x=>sid(x)===String(id));if(!s)return;let old=$("staffDetailModal");if(old)old.remove();const reqs=staffRequests(s);const tags=parseTags(s);const fields=[[T("name"),s.name||"-"],[T("dept"),deptOf(rawDept(s)).code+" "+(lang()==="ja"?deptOf(rawDept(s)).ja:deptOf(rawDept(s)).vi)],[T("email"),s.email||"-"],[T("registered"),s.createdAt?new Date(s.createdAt).toLocaleDateString(lang()==="ja"?"ja-JP":"vi-VN"):"-"],[T("history"),reqs.length]];const modal=document.createElement("div");modal.id="staffDetailModal";modal.className="staff-detail-modal show";modal.onclick=e=>{if(e.target===modal)closeStaffDetail()};modal.innerHTML=`<div class="staff-detail-box"><div class="staff-detail-top"><div class="staff-detail-tabs"><button class="staff-detail-tab active" data-staff-tab="basic">${esc(T("basic"))}</button><button class="staff-detail-tab" data-staff-tab="history">${esc(T("history"))}</button></div><button class="staff-detail-close" onclick="closeStaffDetail()"></button></div><div class="staff-detail-body"><div class="staff-detail-pane active" id="staffDetailPaneBasic"><div class="staff-profile-center">${avatar(s,"staff-avatar-large")}<div class="staff-profile-name">${esc(s.name||"-")}</div><div class="staff-profile-role"><span class="staff-dept-pill final-dept"><b>${esc(deptOf(rawDept(s)).code+" "+(lang()==="ja"?deptOf(rawDept(s)).ja:deptOf(rawDept(s)).vi))}</b></span></div></div><div class="staff-detail-section-title">${esc(T("basic"))}</div><div class="staff-info-grid">${fields.map(([k,v])=>`<div class="staff-info-item"><b>${esc(k)}</b><span>${esc(v)}</span></div>`).join("")}</div><div class="staff-detail-section-title">${esc(T("work"))}</div>${workChipsHtml(tags,99)}</div><div class="staff-detail-pane" id="staffDetailPaneHistory"><div class="staff-detail-section-title">${esc(T("history"))}</div><div class="staff-history-table"><div class="staff-history-row staff-history-head"><div>${esc(T("requestId"))}</div><div>${esc(T("content"))}</div><div>${esc(T("status"))}</div><div>${esc(T("wait"))}</div></div>${reqs.length?reqs.map(r=>`<div class="staff-history-row"><div>${esc(r.requestId||r.id||r._id||"-")}</div><div>${esc(typeof requestTitle==="function"?requestTitle(r):(r.content||"-"))}</div><div>${esc(r.status||"-")}</div><div>${esc(typeof elapsed==="function"?elapsed(r.createdAt):"-")}</div></div>`).join(""):`<div style="padding:16px;color:#64748b">${esc(T("noData"))}</div>`}</div></div></div></div>`;document.body.appendChild(modal)};
  window.closeStaffDetail=function(){$("staffDetailModal")?.remove()};
  function refreshStaffModalLanguage(){if(!$("staffFormModal"))return;renderStaffWorkTagOptionsFinal(staffWorkItemsFinal);const title=document.querySelector("#staffFormModal .staff-form-title");if(title)title.textContent=T("title");const labels=document.querySelectorAll("#staffFormModal .staff-field label");if(labels[0])labels[0].textContent=T("name");if(labels[1])labels[1].textContent=T("email");if(labels[2])labels[2].textContent=T("dept");if(labels[3])labels[3].textContent=T("work");const hint=document.querySelector("#staffFormModal .staff-field .muted");if(hint)hint.textContent=T("hint");const input=$("staffWorkInput");if(input)input.placeholder=T("workPlaceholder");const add=document.querySelector("#staffFormModal .staff-work-add");if(add)add.textContent=T("addWork");const cancel=document.querySelector("#staffFormModal .staff-cancel-profile");if(cancel)cancel.textContent=T("cancel");const save=document.querySelector("#staffFormModal .staff-save-profile");if(save)save.textContent=T("save");const pick=document.querySelector("#staffFormModal .staff-avatar-pick");if(pick)pick.textContent=T("pickAvatar")};
  document.addEventListener("click",e=>{const rm=e.target.closest("[data-remove-work]");if(rm){e.preventDefault();e.stopPropagation();return removeStaffWorkItemFinal(rm.dataset.removeWork)}const add=e.target.closest(".staff-add-btn");if(add){e.preventDefault();return openStaffProfileForm("")}const edit=e.target.closest("[data-edit-staff]");if(edit){e.preventDefault();e.stopPropagation();return openStaffProfileForm(edit.dataset.editStaff)}const del=e.target.closest("[data-delete-staff]");if(del){e.preventDefault();e.stopPropagation();return window.deleteStaffProfile&&window.deleteStaffProfile(del.dataset.deleteStaff)}const row=e.target.closest(".staff-profile-row[data-staff-id]");if(row&&!e.target.closest("button"))openStaffDetail(row.dataset.staffId);const tab=e.target.closest(".staff-detail-tab");if(tab){document.querySelectorAll(".staff-detail-tab").forEach(x=>x.classList.toggle("active",x===tab));document.querySelectorAll(".staff-detail-pane").forEach(p=>p.classList.toggle("active",p.id==="staffDetailPane"+(tab.dataset.staffTab==="basic"?"Basic":"History")))}});const oldApply=window.applyLanguage;window.applyLanguage=function(){if(oldApply)oldApply();setTimeout(()=>{refreshStaffModalLanguage();if((typeof currentView!=="undefined"?currentView:"")==="staff")renderStaff()},0)};document.addEventListener("change",e=>{if(e.target&&e.target.matches("#language,#authLanguage,.language-select,select"))setTimeout(refreshStaffModalLanguage,0)});document.addEventListener("DOMContentLoaded",()=>setTimeout(()=>{refreshStaffModalLanguage();if((typeof currentView!=="undefined"?currentView:"")==="staff")renderStaff()},300));
})();
...
(function(){
  ...
  async function fetchUsersFresh(){const sep=API.indexOf('?')>=0?'&':'?';const res=await apiFetch(API+'/api/admin/users'+sep+'_ts='+Date.now(),{cache:'no-store'});if(res.status===401)return authFailLogout();if(!res.ok)throw new Error('users load failed');const data=await res.json();return Array.isArray(data)?data:(data.users||[])}
  ...
  function renderCard(u){const id=uid(u),status=u.status||'pendingApproval',statusLabel=status==='pendingApproval'?'pending':status,pending=status==='pendingApproval'||status==='pending',blocked=status==='blocked',deleted=status==='deleted',pill=deleted?'red':pending?'orange':blocked?'red':'green';let actions='';if(pending){actions='<button class="approve" data-user-action="approve" data-user-id="'+esc(id)+'">'+T('approve')+'</button><button class="delete" data-user-action="reject" data-user-id="'+esc(id)+'">'+T('reject')+'</button>'}else if(deleted){actions='<button class="reactivate" data-user-action="reactivate" data-user-id="'+esc(id)+'">'+T('reactivate')+'</button><button class="permanent-delete" data-user-action="permanent" data-user-id="'+esc(id)+'">'+T('permanentDelete')+'</button>'}else{actions='<button class="block" data-user-action="'+(blocked?'unblock':'block')+'" data-user-id="'+esc(id)+'">'+(blocked?T('unblock'):T('block'))+'</button><button class="delete" data-user-action="delete" data-user-id="'+esc(id)+'">'+T('delete')+'</button>'}return '<div class="customer-card-v30 admin-user-card-final '+(deleted?'deleted-user':'')+'" data-user-row="'+esc(id)+'"><div><b>'+esc(u.name||u.phone||'-')+'</b><div class="muted">'+esc(u.phone||'-')+' / '+esc(u.email||'-')+'</div><div class="muted">'+esc(u.company||u.customerType||'-')+'</div></div><div><span class="pill '+pill+'">'+esc(labelStatus(statusLabel))+'</span> <span class="pill blue">'+esc(u.province||'-')+'</span><div class="muted">'+esc(u.requestCount||0)+' '+T('requestCount')+'</div></div><div class="customer-actions-v30">'+actions+'</div></div>'}
...
(function(){
  ...
  window.loadUsers=async function(){
    ...
    try{
      ...
      const res=await apiFetch(API+"/api/admin/users"+sep+"_ts="+Date.now(),{cache:"no-store"});
      if(res.status===401)return authFailLogout();
      if(res.ok){
        const data=await res.json();
