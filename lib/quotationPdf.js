...
function normalizeQuoteForPdf(quote = {}) {
  ...
  const items = rawItems.length ? rawItems.map((item, index) => {
    ...
    const discountAmount = numericValue(item.discount, 0);
    const discountRate = numericValue(item.discountRate, 0);
    const beforeDiscount = quantity * unitPrice;
    const computedDiscount = discountAmount || Math.round(beforeDiscount * discountRate / 100);
    ...
    return {
      ...
      quantity,
      unitPrice,
      discountRate: discountRate || (beforeDiscount ? Math.round(discountAmount / beforeDiscount * 100) : 0),
  ...
  const discount = numericValue(quote.discount, 0);
