...
function uploadMediaToCloudinary(file) {
  return new Promise((resolve, reject) => {
    if (!isCloudinaryReady()) {
      return reject(new Error("Cloudinary ENV is missing"));
    }
...
app.get("/admin/users/:id/requests", requireAdmin, async (req, res) => {
  try {
    ...
    console.log("ADMIN USER REQUESTS ERROR:", error);
    res.status(500).json({ message: "Read failed", error: error.message });
  }
});
...
app.post("/request", (req, res) => {
  upload.array("image", 8)(req, res, async function (uploadError) {
    if (uploadError) {
      console.log("MULTER UPLOAD ERROR:", uploadError);
    ...
    try {
      ...
      console.log("REQUEST FILES:", (req.files || []).map(file => ({
...
app.get("/requests", requireAdmin, async (req, res) => {
  try {
    ...
    console.log("REQUESTS READ ERROR:", error);
    res.status(500).json({ message: "Read failed", error: error.message });
  }
});
...
app.get("/request/:id", async (req, res) => {
  try {
    ...
    console.log("REQUEST READ ERROR:", error);
    res.status(500).json({ message: "Read failed", error: error.message });
  }
});
