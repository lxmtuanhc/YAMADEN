...
function normalizeQuote(quote: Quote): Quote {
  ...
  return {
    ...
    ext: quote.ext || "",
    quoteFiles: quote.quoteFiles || [],
    quotationFiles: quote.quotationFiles || [],
    quoteSent: quote.quoteSent === true || Boolean(quote.fileUrl) || Boolean(quote.quoteFiles?.length) || Boolean(quote.quotationFiles?.length),
    quoteSentAt: quote.quoteSentAt || quote.sentAt || quote.sentToCustomerAt || "",
    quoteSentBy: quote.quoteSentBy || "",
    quoteFileCount: quote.quoteFileCount || quote.quoteFiles?.length || quote.quotationFiles?.length || (quote.fileUrl ? 1 : 0),
    sentAt: quote.sentAt || "",
